...
3. If `.env` does not exist, copy `.env.images.example` to `.env`. Fill secrets with openssl (`rand -hex 16` for `POSTGRES_PASSWORD`, `rand -hex 32` for `BETTER_AUTH_SECRET`, `ENCRYPTION_KEY`, and `SCREEN_PROXY_SECRET`). Keep `SANDBOX_PROVIDER=none` unless I chose a remote computer provider. Add only the model key I selected.
4. Run:
